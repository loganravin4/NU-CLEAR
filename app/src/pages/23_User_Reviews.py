...
if user_id:
    ...
    if reviews:
        ...
        review_id = st.selectbox("Select a review to edit or delete", options=[review['reviewId'] for review in reviews], key="review_id")
        ...
        if review_id:
            ...
            with st.form("update_review_form"):
                ...
                summary = st.text_area("Summary", value=selected_review['summary'])
                best_part = st.text_area("Best Part", value=selected_review['bestPart'])
                worst_part = st.text_area("Worst Part", value=selected_review['worstPart'])
                rating = st.slider("Rating", min_value=0.0, max_value=5.0, value=selected_review['rating'], step=0.5)
